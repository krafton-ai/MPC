...
m1_model_type = "gpt" # modular gpt
m2_model_type = "alt_gpt" # plain gpt
...
utils.init_pair_persona(m1_config_path, m2_config_path, st.session_state, init_past=True)
...
# st.session_state['bot_start'] = random.choice(conversation_starter)
...
st.sidebar.markdown(PAIR_INSTRUCTIONS_SESSIONS)
...
if st.session_state['start_page']:
    ...
    if authentication_status:
        with st.form('start', clear_on_submit=True):
            st.markdown(PAIR_INSTRUCTIONS_SESSIONS)
            st.markdown(CONSENT)
    ...
    if st.session_state['eval_mode']:
        ...
        with st.form('eval', clear_on_submit=True):
            ...
            if submitted and allselected:
                ...
                if len(st.session_state['pair/sensibleness']) >= n_turns:
